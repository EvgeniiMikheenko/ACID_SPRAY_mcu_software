...
#define MOBUS_PACKET_SIZE	1000
#define NULL		0
#define DEVICE_INFO 	0

#define RE_ON			HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_SET)	
#define RE_OFF		HAL_GPIO_WritePin(GPIOA, GPIO_PIN_15, GPIO_PIN_RESET)
...
void rs485_start_tx( uint8_t data )
{
	...
	HAL_UART_Transmit_IT( &huart3, m_mbParam.lpTxBuf, 1);
...
void mb_tx( MbDeviceData_t *Params )
{
	 ...
	 //
			HAL_UART_Transmit_IT( &huart3, &Params->lpTxBuf[Params->txIndex++], 1);
		else 
		{
